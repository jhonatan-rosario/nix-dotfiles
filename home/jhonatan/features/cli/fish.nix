...
in {
  ...
  programs.fish = let
    flakePath = "~/Workspace/nix-config";
  in {
    enable = true;
    shellAbbrs = rec {
      jqless = "jq -C | less -r";
      ...
      n = "nix";
      nd = "nix develop -c $SHELL ${flakePath}";
      ns = "nix shell";
      ...
      nr = "sudo nixos-rebuild --flake ${flakePath}";
      nrs = "sudo nixos-rebuild --flake ${flakePath} switch";
      # snr = "sudo nixos-rebuild --flake ${flakePath}";
      # snrs = "sudo nixos-rebuild --flake ${flakePath} switch";
